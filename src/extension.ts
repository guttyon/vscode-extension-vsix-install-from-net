...
export function activate(context: vscode.ExtensionContext) {
	...
	let disposable = vscode.commands.registerCommand('vsix-install-from-net.install', async () => {
		...
		async function dl_and_install(url : string){
			...
			//let cmdstr_dl = [`curl -o $env:temp/vscode-extension-vsix-install-from-net/ ${url}`];
			...
			let cmdstr_del = [`del $env:temp/vscode-extension-vsix-install-from-net/`];

			await term.processId;
			//await Utils.delay ( 200 );
			for(let cmdstr of cmdstr_dl){
				term.sendText(cmdstr);
			}
			if(1){
				for(let cmdstr of cmdstr_install){
					term.sendText(cmdstr);
				}
			}

			//term.sendText ( `code --install-extension "${file.fsPath}" ${force ? '--force' : ''}` );
